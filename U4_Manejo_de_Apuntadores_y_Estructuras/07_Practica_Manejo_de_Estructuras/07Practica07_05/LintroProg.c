...
//char (*funcion_LIntroProg_Inv(short ndl))[]
/** */
char_array_pt_t ImagenInversaD_LIntroProg(struct ConjDInts CdI)
{
 short index;
 char_array_pt_t char_array_pt = NULL;
 static char result[16*SAMPLE_SPACE_CARDINALITY+2];
 for (index=0;index<16*SAMPLE_SPACE_CARDINALITY+2;index++){
   result[index] = '\0';
 }
 result[0] = '{';result[1] = ' ';result[2] = '}';
 if (CdI.canDElems > 0){
   char_array_pt = funcion_LIntroProg_Inv(CdI.Elem[0]);
   strcpy(result,*char_array_pt);
   for(index=1;index<CdI.canDElems;index++){
     char_array_pt = funcion_LIntroProg_Inv(CdI.Elem[index]);
     char_array_pt = union_de_dias(result,*char_array_pt);
     strcpy(result,*char_array_pt);
   }
 }
 return &result;
}/*end ImagenInversaD_LIntroProg()*/

/**Stub
 * char evt[] = "{LUNES,MIERCOLES}";
 * struct ConjDInts *CdI_Pt = ImagenDirectaD_LIntroProg(&evt);
 * printf("ImagenDirectaD_LIntroProg(%s) = ",evt);
 * printf("{");print_ConjDInt(*CdI_Pt,',');printf("}\n");
 * Para el evento {LUNES,MIERCOLES}, debemos obtener:
 * ImagenDirectaD_LIntroProg({LUNES,MIERCOLES}) = {4}
 *
 * ImagenDirectaD_LIntroProg({LUNES,MIERCOLES,JUEVES}) = {4,1}
 * ImagenDirectaD_LIntroProg({MARTES}) = {0}
 * ImagenDirectaD_LIntroProg({LUNES,MARTES}) = {4,0}
 * ImagenDirectaD_LIntroProg({LUNES,MARTES,MIERCOLES}) = {4,0}
 * ImagenDirectaD_LIntroProg({LUNES,MARTES,MIERCOLES,JUEVES}) = {4,0,1}
 * ImagenDirectaD_LIntroProg({LUNES,MARTES,MIERCOLES,JUEVES,VIERNES}) = {4,0,1}
 * ImagenDirectaD_LIntroProg({LUNES,MARTES,MIERCOLES,JUEVES,VIERNES,SABADO}) = {4,0,1}
 * ImagenDirectaD_LIntroProg({LUNES,MARTES,MIERCOLES,JUEVES,VIERNES,SABADO,DOMINGO}) = {4,0,1}
 * ImagenDirectaD_LIntroProg({ }) = {};
 */
struct ConjDInts *
ImagenDirectaD_LIntroProg(char_array_pt_t char_array_pt)
{
 struct ConjDInts *result = NULL;
 result = (struct ConjDInts *)malloc(sizeof(*result));
 /**Determinar el valor que deber\'a tener result->canDElems,
  * Inicializar (usando la funci\'on malloc) result->Elem,
  * Colocar en result->Elem[i] los valores correspondientes.
  */
